...
class Connect_twitter extends CI_Controller {
	...
	function index()
	{
            // Fission added, ?continue lost between here and signin (during auth), so set session
            if ($this->input->get('continue'))
                $this->session->set_userdata('sign_in_redirect', $this->input->get('continue'));
            if ($this->input->post('continue'))
                $this->session->set_userdata('sign_in_redirect', $this->input->post('continue'));
			...
			else
			{
				...
				if ( ! $this->authentication->is_signed_in())
				{
					...
					// Create a3m account
                                        ...
                                        // Fission change
                                        redirect('site/account_connect_create');
				}
				else
				{
