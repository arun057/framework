...
class Connect_create extends CI_Controller {
	...
	function index()
	{
		...
		$this->form_validation->set_rules(array(
			...
			array('field'=>'connect_create_firstname', 'label'=>'lang:connect_create_firstname', 'rules'=>'trim|required|max_length[160]'),
			array('field'=>'connect_create_lastname', 'label'=>'lang:connect_create_lastname', 'rules'=>'trim|required|max_length[160]'),
			array('field'=>'connect_create_email', 'label'=>'lang:connect_create_email', 'rules'=>'trim|required|valid_email|max_length[160]')
		));
		...
		if ($this->form_validation->run()) 
		{
			...
			else
			{
				...
				$user_id = $this->account_model->create($this->input->post('connect_create_username'), $this->input->post('connect_create_email'));
                                ...
                                if ($this->input->post('connect_create_firstname'))
                                    $data['connect_create'][1]['firstname'] = $this->input->post('connect_create_firstname');

                                if ($this->input->post('connect_create_lastname'))
                                    $data['connect_create'][1]['lastname'] = $this->input->post('connect_create_lastname');
