...
class Account_model extends CI_Model {
	 ...
	 *
	 */
	function create($username, $email = NULL, $password = NULL, $firstname = NULL, $lastname = NULL)
	{
		// Changed by Fission (BM) to delegate password hashing
		// to the Authentication module.
		if ($password !== NULL) {
			$this->load->library('authentication');
			$hashed_password = $this->authentication->hash($password);
		}
		...
		// Commented out by Fission.
		/* Replace this with ActionKit API
		try {
		    ...
		    $this->salesforce->add_contact($contact);
		} catch (Exception $e) {
		    error_log("unable to add $username $email to Salesforce " . $e->getMessage());
		}
		*/
	...
	{
		$this->load->library('authentication');
		$new_hashed_password = $this->authentication->hash($password_new);
		$this->db->update('a3m_account', array('password' => $new_hashed_password), 
						  array('id' => $account_id));
	}
