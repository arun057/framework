...
(function($){
	...
	$.addFlex = function(t,p)
	{
			...
			populate: function () { //get latest data
				...
				if (p.params)
					...
					$.ajax({
					   ...
					   data: param,
					   dataType: p.dataType,
					   success: function(data){g.addData(data); $('#flexgrid').parent().css('height',($('#flexgrid').attr('clientHeight')+100)); },
		...
		//add search button
		//if (p.searchitems=='RICKDISABLED')
                col_names = p.allitems;
		if (p.searchitems)
			{
                            if (0) {  // added if 0  to hide the magnifying glass - search is moved to toolbar later - cindy
				...
				$('input[value=Clear]',g.sDiv).click(function(){$('input[name=q]',g.sDiv).val(''); p.query = ''; g.doSearch(); });
                                ...
                                // move search into top toolbar - cindy
                                // $(g.bDiv).after(g.sDiv);				
                                $('.tDiv2').prepend(g.sDiv);				
				
			}
			else
			{
				//Rick Add Here
				$('.pDiv2',g.pDiv).prepend("<div class='btnseparator'></div>");	
			}
